...
一般来说，如果你使用的是比较朴素的 13E ln 和 17E exp，得到的就是一个 30E 的 pow 算法；如果你选择的是 SOTA 10E ln[^1] + 11.5E exp [^2]，你将收获一个 21.5E 的 pow；根据我的实践，10E ln + 14E exp 在实机上表现最好，也就是 24E 的 pow。

也有独立的 pow 算法，目前的 SOTA 是 20.25E [^2]，不过太复杂了就没尝试实现，目测常数极大。

本文提出了一种新的 18E 的计算多项式幂的算法。

## 分块

建议阅读 [多项式牛顿迭代的分块优化](/zh/post/2022/poly-newton/)，本文的分块思路是其延续。
...
为了方便起见，我们记 $\Delta_i f = X^{-i} \Delta(X^i f)$，也就是一个平移的微分算子，有
...
严格的说，当块数 $B = O(\sqrt{\log{n}})$ 的时候取得平衡，复杂度 $T(n) \leqslant 18\mathsf{E}(n) + O(n \sqrt{\log n})$。
...
代码：[yosupo-pow](https://judge.yosupo.jp/submission/382181)。

## 总结

本来只是闲暇时的瞎想，没想到越琢磨越有道理，终有此篇。也总算是做出来一点点属于自己的东西了。

[^1]: [Fast algorithms for elementary operations on complex power series](https://doi.org/10.1515/DMA.2010.002)

[^2]: [A note on the fast power series' exponential](https://arxiv.org/abs/1203.3883)
